...
class PySysTest(EthereumTest):
    ...
    def execute(self):
        ...
        deploy_balance_l1 = obx_cntr_l1.functions.balanceOf(deploy_account_l1.address).call()
        bridge_balance_l1 = obx_cntr_l1.functions.balanceOf(bridge_address).call()
        ...
        with open(os.path.join(PROJECT.root, 'utils', 'contracts', 'erc20', 'erc20.json')) as f:
            obx_cntr_l2 = web3_l2.eth.contract(address=Properties().l2_obx_token_address(l2.PROPS_KEY), abi=json.load(f))

        deploy_balance_l2 = obx_cntr_l2.functions.balanceOf(deploy_account.address).call()
        bridge_balance_l2 = obx_cntr_l2.functions.balanceOf(bridge_address).call()
        ...
        if deploy_balance_l2 < self.THRESHOLD:
            ...
            l1.transact(self, web3_l1, obx_cntr_l1.functions.transfer(bridge_address, amount), deploy_account, 7200000)

            deploy_balance_l1_after = obx_cntr_l1.functions.balanceOf(deploy_account.address).call()
            bridge_balance_l1_after = obx_cntr_l1.functions.balanceOf(bridge_address).call()
            ...
            time.sleep(30)
            deploy_balance_l2_after = obx_cntr_l2.functions.balanceOf(deploy_account.address).call()
            bridge_balance_l2_after = obx_cntr_l2.functions.balanceOf(bridge_address).call()
