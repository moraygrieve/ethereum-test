...
class PySysTest(BaseTest):
    USERS = {
        ...
        'GAVIN':'0x6D0c4F15c048Efef3656F77e393C8cc149aE9262'
    }
    AMOUNT = 50
    ...
    def execute(self):
        ...
        with open(os.path.join(PROJECT.root, 'utils', 'contracts', 'erc20', 'erc20.json')) as f:
            contract_l2 = web3_l2.eth.contract(address=Properties().l2_obx_token_address(l2.PROPS_KEY),
                                               abi=json.load(f))
        # run for users
        for user in self.USERS.keys():
            ...
            user_balance = contract_l2.functions.balanceOf(user_address).call()
            deploy_balance = contract_l2.functions.balanceOf(deploy_account.address).call()
            ...
            if user_balance == 0:
                ...
                l2.transact(self, web3_l2, contract_l2.functions.transfer(user_address, self.AMOUNT), deploy_account, 7200000)
